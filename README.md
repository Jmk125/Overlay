...
- **Batch or single overlay** — load individual PDFs or multi-page drawing sets; large sets can be loaded by typing a page range up front (e.g. `1-20, 45`), while the page picker auto-loads thumbnail previews in parallel when you need to choose visually
- **Four matching modes** — auto-match by **OCR** (zoom/pan the page, draw a box around the sheet number; OCR runs across all CPU cores for large sets), match by each PDF's own **page label** (Acrobat/Bluebeam "Page Labels", for sets already scanned and labeled to correspond), match by **PDF page order**, or pair **manually**
...
- **Highlight tool** — draw a box or freeform area and recolor the drawing's own linework inside it (to your markup color) instead of drawing a shape on top — e.g. highlight the walls coming out in a demo phase; shows whatever's currently on screen (both drawings in the overlay view, just one in A-only/B-only)
