...
class WorkspaceCanvas(QGraphicsView):
    ...
    def _item_at_event(self, event):
        ...
        while item and not isinstance(item, WorkspacePixmapItem):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            item = self.selected_item()
            if item and self._mode == self.MODE_MOVE:
                self._drag_start = self.mapToScene(event.position().toPoint())
                return
            if item and self._mode == self.MODE_ROTATE:
                self._rotate_start = self.mapToScene(event.position().toPoint())
                return
